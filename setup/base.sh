...
# Handle Copilot installation
if [ "$COPILOT" = true ]; then
    echo ""
    echo "📥 Enabling GitHub Copilot support..."

    # Only update config to enable copilot
    if [ -f "$INSTALL_DIR/config.yml" ]; then
        sed -i.bak '/copilot:/,/enabled:/ s/enabled: false/enabled: true/' "$INSTALL_DIR/config.yml" && rm "$INSTALL_DIR/config.yml.bak"
...
if [ "$COPILOT" = true ]; then
    echo "   GitHub Copilot support enabled in configuration"
fi
